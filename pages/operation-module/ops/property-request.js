...
onLoad:function(c){
  c.innerHTML=_PREQ_html();
  _OP_wireCards(c);_OP_wireSearch(c);_OP_wireAddBtns(c);
  _PREQ_wireActions(c);
  _PREQ_loadHistory(c);
},onUnload:function(){}};

function _PREQ_wireActions(container) {
  const saveBtn  = container.querySelector('#preq-save');
  const resetBtn = container.querySelector('#preq-reset');
  if (saveBtn) saveBtn.addEventListener('click', async function () {
    saveBtn.disabled = true;
    try {
      const num = (document.getElementById('preq-no') || {}).value ||
                  ('PREQ-' + new Date().getFullYear() + '-' + Date.now().toString().slice(-6));
      await _OP_saveHeader({
        table: 'property_requests',
        fields: [
          { id: 'preq-by',       db: 'requested_by', fk: 'profiles' },
          { id: 'preq-purpose',  db: 'remarks' }
        ],
        extra: {
          request_number: num,
          status:         'Pending'
      ...
      Toast.show('Property request submitted.', 'success');
      _PREQ_loadHistory(container);
    } catch (err) {
      console.error('[preq] save failed', err);
...
function _PREQ_loadHistory(container) {
  return _OP_loadHistory({
    table:     'property_requests',
    container: container,
    tbodyId:   'preq-rec-tbody',
    columns:   ['Request No.','Requested By','Status','Purpose','Created'],
    select:    'id, request_number, status, remarks, created_at, requester:profiles!property_requests_requested_by_fkey(full_name, email)',
    rowFn: function (r) {
...
${_card('preq-det','<path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6z"/><polyline points="14 2 14 8 20 8"/>','Request Details','',`
  ...
  <div class="op-form-grid">
    <div>
      ${_field('Request No.','text','preq-no','Leave blank to auto-generate',false)}
    ...
    <div>
      ${_fieldDate('Date Requested','preq-date',false,new Date().toISOString().slice(0,10))}
...
<div class="op-action-bar"><div class="op-action-bar__left">
  <button class="op-btn op-btn--primary" id="preq-save">${_icon('<path d="M22 2L11 13"/><path d="M22 2L15 22l-4-9-9-4 20-7z"/>')} Submit Request</button>
  <button class="op-btn op-btn--secondary" id="preq-reset">${_icon('<polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 102.13-9.36L1 10"/>')} Reset</button>
...
${_card('preq-rec','<line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/>','Property Request Records','',`
  ...
  <div class="dash-table-wrap dash-table-wrap--scroll">
    <table class="dash-tbl" style="min-width:700px;">
      ...
      <tbody id="preq-rec-tbody"><tr><td colspan="5" class="dash-empty">Loading…</td></tr></tbody>
    </table>
